...
export async function saveGameReview(usuario, id){
    /*
        _id
        nombre
        comentario
    */
    await db.collection("juegos").updateOne(
        { _id: new ObjectId(id) },
        { $push: { reviews: { ...usuario } } } //https://www.mongodb.com/es/docs/manual/reference/operator/update/push/
    )
    const juego = await getGameById(id)
    await db.collection("usuarios").updateOne(
        { _id: new ObjectId(usuario._id) },
        { $push: { reviews: { ...juego } } } //https://www.mongodb.com/es/docs/manual/reference/operator/update/addtoset/
    )
    return juego
}